...
import { CollectionInterface, User } from "../../common/interfaces";
...
export default function Collection(props: { collection: CollectionInterface }) {
  ...
  const user = selectUserByID(state, userId) as User;
  ...
  return (
    <CardWrapper onClick={() => navigate(`/${user.id}/${id}`)}>
      ...
      <Typography variant="subtitle2" color={grey[400]}>
        <TranslatedText text="collectionOf" /> <TranslatedText text={type} />
      </Typography>
      <Typography
        variant="subtitle2"
        color={grey[400]}
